...
}).describe("Performs one or more search-and-replace operations within a target Obsidian note (file path, active, or periodic). Reads the file, applies replacements sequentially in memory, and writes the modified content back, overwriting the original. Supports string/regex search, case sensitivity toggle, replacing first/all occurrences, flexible whitespace matching (non-regex), and whole word matching.");
// ====================================================================================
 ...
 */
async function getFinalState(targetType, effectiveFilePath, period, obsidianService, context) {
    ...
    try {
        ...
        if (targetType === "filePath" && effectiveFilePath) {
            noteJson = (await obsidianService.getFileContent(effectiveFilePath, "json", context));
        }
 ...
 */
export const processObsidianSearchReplace = async (params, // Use the refined, validated type
context, obsidianService, vaultCacheService) => {
    // Destructure validated parameters for easier access
    const { targetType, targetIdentifier, replacements, useRegex: initialUseRegex, // Rename to avoid shadowing loop variable
    replaceAll, caseSensitive, flexibleWhitespace, // Note: Cannot be true if initialUseRegex is true (enforced by schema)
    wholeWord, returnContent, } = params;
    ...
    try {
        if (targetType === "filePath") {
            ...
            try {
                ...
                originalContent = (await obsidianService.getFileContent(targetIdentifier, "markdown", readContext));
                ...
                if (readError instanceof McpError &&
                    ...
                    const matches = filesInDir.filter((f) => !f.endsWith("/") &&
                    ...
                    if (matches.length === 1) {
                        ...
                        originalContent = (await obsidianService.getFileContent(effectiveFilePath, "markdown", readContext));
                        ...
                        throw new McpError(matches.length > 1
                            ? BaseErrorCode.CONFLICT
                            : BaseErrorCode.NOT_FOUND, errorMsg, readContext);
                    }
                }
                else {
            ...
            originalContent = (await obsidianService.getActiveFile("markdown", readContext));
            ...
            originalContent = (await obsidianService.getPeriodicNote(targetPeriod, "markdown", readContext));
    ...
    for (let i = 0; i < replacements.length; i++) {
        ...
        let finalSearchPattern = rep.search; // Start with the raw search string
        ...
        try {
            ...
            if (!initialUseRegex) {
                ...
                const hasBoundary = /(?:^|\\b)\S.*\S(?:$|\\b)|^\S$|^\S.*\S$|^$/.test(searchStr) ||
                    /^\^|\\b/.test(searchStr) ||
                    /\$|\\b$/.test(searchStr);
                if (!hasBoundary) {
            ...
            if (useRegexForThisRep) {
                ...
                let flags = "";
                if (replaceAll)
                    flags += "g"; // Global flag for all matches
                if (!caseSensitive)
                    flags += "i"; // Ignore case flag
                const regex = new RegExp(finalSearchPattern, flags); // Create RegExp object
                ...
                const searchString = finalSearchPattern; // It's just a string here
                ...
                while (true) {
                    ...
                    if (rep.replace.includes(searchString) &&
    ...
    if (modifiedContent !== originalContent) {
        ...
        try {
            ...
            try {
                finalState = await retryWithDelay(async () => getFinalState(targetType, effectiveFilePath, targetPeriod, obsidianService, context), {
                    ...
                    onRetry: (attempt, err) => logger.warning(`getFinalStateAfterSearchReplaceWrite (attempt ${attempt}) failed. Error: ${err.message}. Retrying...`, writeContext),
                });
            }
            catch (retryError) {
                finalState = null;
                logger.error(`Failed to retrieve final state for ${targetDescription} after write, even after retries. Error: ${retryError.message}`, retryError instanceof Error ? retryError : undefined, writeContext);
            }
        }
        catch (error) {
            ...
            finalState = await retryWithDelay(async () => getFinalState(targetType, effectiveFilePath, targetPeriod, obsidianService, context), {
                ...
                shouldRetry: (err) => err instanceof McpError &&
                    (err.code === BaseErrorCode.NOT_FOUND ||
                        err.code === BaseErrorCode.SERVICE_UNAVAILABLE ||
                        err.code === BaseErrorCode.TIMEOUT),
                onRetry: (attempt, err) => logger.warning(`getFinalStateAfterSearchReplaceNoChange (attempt ${attempt}) failed. Error: ${err.message}. Retrying...`, context),
            });
        }
        catch (retryError) {
            finalState = null;
            logger.error(`Failed to retrieve final state for ${targetDescription} (no change), even after retries. Error: ${retryError.message}`, retryError instanceof Error ? retryError : undefined, context);
        }
    }
