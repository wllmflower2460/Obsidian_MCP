...
import { dateParser, formatTimestamp, logger, retryWithDelay, sanitizeInputForLogging, } from "../../../utils/index.js";
// ====================================================================================
...
const ObsidianGlobalSearchInputSchema = z
    ...
    searchInPath: z
        .string()
        .optional()
        .describe("Optional vault-relative path to recursively search within (e.g., 'Notes/Projects'). If omitted, searches the entire vault."),
    contextLength: z
        .number()
        .int()
        .positive()
    ...
    modified_since: z
        .string()
        .optional()
        .describe("Filter files modified *since* this date/time (e.g., '2 weeks ago', '2024-01-15')."),
    modified_until: z
        .string()
        .optional()
        .describe("Filter files modified *until* this date/time (e.g., 'today', '2024-03-20 17:00')."),
    useRegex: z
        .boolean()
        .optional()
        .default(false)
...
function findMatchesInContent(content, query, useRegex, caseSensitive, contextLength, context) {
    const matches = [];
    let regex;
...
export const processObsidianGlobalSearch = async (params, context, obsidianService, vaultCacheService) => {
    ...
    let untilDate = null;
    let strategyMessage = "";
    let allFilteredResults = []; // Store all results matching filters before pagination
    ...
    try {
        if (params.modified_since)
            sinceDate = await dateParser.parseToDate(params.modified_since, dateParseContext);
        if (params.modified_until)
            untilDate = await dateParser.parseToDate(params.modified_until, dateParseContext);
    }
    catch (error) {
    ...
    try {
        ...
        const apiResults = await retryWithDelay(async () => {
            logger.info(`Calling obsidianService.searchSimple for query: "${params.query}"`, apiSearchContext);
            const apiCallPromise = obsidianService.searchSimple(params.query, params.contextLength, apiSearchContext);
            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error(`API search timed out after ${API_SEARCH_TIMEOUT_MS}ms`)), API_SEARCH_TIMEOUT_MS));
        ...
        for (const apiResult of apiResults) {
            ...
            try {
                const noteJson = (await obsidianService.getFileContent(filePathFromApi, "json", fetchStatsContext));
                mtime = noteJson.stat.mtime;
                ctime = noteJson.stat.ctime; // Get ctime
                // Apply date filtering if needed
                if ((sinceDate && mtime < sinceDate.getTime()) ||
    ...
    if (apiFailedOrTimedOut) {
        if (vaultCacheService && vaultCacheService.isReady()) {
            ...
            for (const [filePath, cacheEntry] of cache.entries()) {
                ...
                try {
                    const matches = findMatchesInContent(cacheEntry.content, params.query, params.useRegex, params.caseSensitive, params.contextLength, cacheSearchContext);
                    // Apply match limit per file
                    ...
                    if (limitedMatches.length > 0) {
                        ...
                        try {
                            const noteJson = (await obsidianService.getFileContent(filePath, "json", cacheSearchContext));
            ...
            logger.error(`API search failed and cache ${reason}. Cannot perform search.`, opContext);
            // Throw a specific error because the tool cannot function without a data source.
            throw new McpError(BaseErrorCode.SERVICE_UNAVAILABLE, `Live API search failed and the cache is currently ${reason}. Please ensure the Obsidian REST API is running and reachable, and that the cache is enabled and has had time to build.`, opContext);
        }
    }
    ...
    let alsoFoundInFiles = undefined;
    if (totalPages > 1) {
